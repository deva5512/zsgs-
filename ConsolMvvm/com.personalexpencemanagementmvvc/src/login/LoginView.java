...
public class LoginView {
	...
	private void dailyCalculator() {
		...
		loginModel.dailyCalculator(balance, percent);
	}
	...
	public void UserCreatedSuccessfully(String name, int pin) {
		...
		while (true) {
			...
			boolean validation = loginModel.checkPosible(balance, fixedexpence, percent);
			if (validation) {
				loginModel.setAccountBalance(balance, fixedexpence, percent);
				break;
			} else {
	...
	private void history() {
		...
		if (input == 1) {
			history = loginModel.getCustomhistory(LocalDate.now() + "");
		} else if (input == 2) {
			...
			history = loginModel.getCustomhistory(date + "");
		} else if (input == 3) {
			history = loginModel.getCompletehistory();
		} else {
	...
	private void enterDailyExpence() {
		...
		double allowed = loginModel.getUser().getAllowedonedayamount();

		double spent = loginModel.getUser().getTodayspending();
	...
	private void modifyUserDetails() {
		while (true) {
			...
			if (input == 1) {
				...
				loginModel.modifyFixedExpence(amount);
			} else if (input == 6) {
				break;
			} else {
				System.out.println("\nInvalid input ");
	...
	private void modifySavingPercent() {
		while (true) {
			...
			if (input == 1) {
				...
				loginModel.modifyPercent(percent);

			} else if (input == 2) {
				int percent = GetInput.getInt("percent to reduce");

				loginModel.modifyPercent(percent * -1);
			} else {
				if (input != 3) {
					System.out.println("\nImproper input : ");
				}
	...
	private void modifyBalance() {
		while (true) {
			...
			if (input == 1) {
				...
				loginModel.modifyBalance(amount);

			} else if (input == 2) {
				double amount = GetInput.getDouble("amount you need to reduce");

				loginModel.modifyBalance(amount * -1);
			} else {
				if (input != 3) {
					System.out.println("\nImproper input : ");
				}
